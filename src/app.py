...
class RegistrationModule:
    def __init__(self, logFileName):
        ...
        self.window.configure(background='#808000')

        self.window.grid_rowconfigure(0, weight=1)
        self.window.grid_columnconfigure(0, weight=1)

        header = tk.Label(self.window,text="User Face Recognition", width = 60,height= 2,
                          fg="white", bg = "black",font=('times',18,'bold','underline'))
        header.place(x=0,y=0)


        empName = tk.Label(self.window, text="User Name : ", width=10, fg="white",bg='#808000', height=1,
                           font=('times', 15))
        empName.place(x=100, y=160)

        self.empNameTxt = tk.Entry(self.window, width=20, bg="white", fg="black", font=('times', 15, ' bold '))
        self.empNameTxt.place(x=205, y=160)



        lbl3 = tk.Label(self.window, text="Notification : ", width=15,bg='#808000',  fg="white", height=2,
                        font=('times', 15))

        lbl3.place(x=80, y=250)

        self.message = tk.Label(self.window, text="", bg="#C0C0C0", fg="red", width=50, height=2,
                                activebackground="#bbc7d4",
                                font=('times', 15))
        self.message.place(x=218, y=250)

        takeimage = tk.Button(self.window,text="Take Images", command=self.collectUserImageForRegistration, fg="white",
                              bg="#008000", width=15, height = 2, activebackground="#118ce1",font=('times',15,'bold'))
        takeimage.place(x=80, y=350)
        trainImg = tk.Button(self.window, text="Train Images", command=self.trainModel, fg="white", bg="#008000",
                             width=15,
                             height=2,
                             activebackground="#118ce1", font=('times', 15, ' bold '))
        trainImg.place(x=350, y=350)
        predictImg = tk.Button(self.window, text="Predict", command=self.makePrediction, fg="white", bg="#008000",
                               width=15,
                               height=2,
                               activebackground="#118ce1", font=('times', 15, ' bold '))
        ...
        quitWindow = tk.Button(self.window, text="Quit",command=self.close_window, fg="white", bg="red", width=10,
                               height=2,
                               activebackground="#118ce1", font=('times', 15, 'bold'))
    ...
    def getFaceEmbedding(self):
        ...
        ap.add_argument('--image-size', default="112,122", help='')
        ap.add_argument("--model", default='../insightface/models/model-y1-test2/model,0', help="path to load model")
        ap.add_argument('--ga-model', default='', help='path to load model')
        ap.add_argument('--gpu', default=0, type=int, help='gpu id')
        ap.add_argument('--det', default=0, type=int,
                        help='mtcnn option, 1 mean using R+0, 0 mean dectect from begining')
        ap.add_argument('--flip', default=0,type=int,help='whether do lr flip aug')
        ap.add_argument('--thresold', default=1.24,type=float,help='ver dist thresold')
        args = ap.parse_args()

        genFaceEmbedding = GenerateFaceEmbedding(args)
        genFaceEmbedding.genFaceEmbedding()
...
RegistrationModule(logFileName)
